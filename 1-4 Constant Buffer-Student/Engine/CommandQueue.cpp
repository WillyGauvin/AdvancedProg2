...
void CommandQueue::Init(ComPtr<ID3D12Device> device, shared_ptr<SwapChain> swapChain)
{
	...
	*/
	...
	/*CreateFence
	 - CPU GPU */
	 //CALL CreateFence() on device passing in 0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&_fence) //DONE
	device->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&_fence));
	//ASSIGN _fenceEvent to the return value of ::CreateEvent() passing in "nullptr, FALSE, FALSE, nullptr" to CreateEvent() //DONE
...
//INCREMENT _fenceValue //DONE
	_fenceValue++;

	/* Add an instruction to the command queue to set a new fence point.  Because we
	 are on the GPU timeline, the new fence point won't be set until the GPU finishes
	 processing all the commands prior to this Signal(). */
	 //CALL Signal() on _cmdQueue passing in _fence.Get(), _fenceValue //DONE
	_cmdQueue->Signal(_fence.Get(), _fenceValue);

	/* Wait until the GPU has completed commands up to this fence point. */
	//IF _fence->GetCompletedValue() LESS THAN _fenceValue //DONE
	if (_fence->GetCompletedValue() < _fenceValue)
	{
		/* Fire event when GPU hits current fence. */
		//CALL SetEventOnCompletion() on _fence passing in _fenceValue, _fenceEvent //DONE  
		_fence->SetEventOnCompletion(_fenceValue, _fenceEvent);

		/* Wait until the GPU hits current fence event is fired. */
		//CALL ::WaitForSingleObject() passing in _fenceEvent, INFINITE //DONE
...
void CommandQueue::RenderBegin(const D3D12_VIEWPORT* vp, const D3D12_RECT* rect)
{
	...
	//DECLARE a variable of type D3D12_RESOURCE_BARRIER called barrier and ASSIGN it to the return value of CD3DX12_RESOURCE_BARRIER::Transition(), pass in _swapChain->GetCurrentBackBufferResource().Get(), D3D12_RESOURCE_STATE_PRESENT, D3D12_RESOURCE_STATE_RENDER_TARGET to Transition()  
	...
	//SET the Graphics Root Signature on the command List
	_cmdList->SetGraphicsRootSignature(GEngine->GetRootSignature()->GetSignature().Get());
	
	//CLEAR the Constant Buffer, clear it via GEngine
	GEngine->GetCB()->Clear();
	//END STEP 3 NEW
	...
	//CALL OMSetRenderTargets() on _cmdList passing in 1, &backBufferView, FALSE, nullptr DONE
	_cmdList->OMSetRenderTargets(1, &backBufferView, FALSE, nullptr);
}

void CommandQueue::RenderEnd()
{
	//DECLARE a variable of type D3D12_RESOURCE_BARRIER called barrier and ASSIGN it to the return value of CD3DX12_RESOURCE_BARRIER::Transition() passing in _swapChain->GetCurrentBackBufferResource().Get(),	D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_PRESENT to Transition() DONE
	D3D12_RESOURCE_BARRIER barrier = CD3DX12_RESOURCE_BARRIER::Transition(
		_swapChain->GetBackRTVBuffer().Get(),
		D3D12_RESOURCE_STATE_RENDER_TARGET, // 
		D3D12_RESOURCE_STATE_PRESENT); //
