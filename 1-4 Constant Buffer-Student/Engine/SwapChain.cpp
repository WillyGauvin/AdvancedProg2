...
{
	//CALL CreateSwapChain() passing in info, dxgi, cmdQueue
	CreateSwapChain(info, dxgi, cmdQueue);
	//CALl CreateRTV() passing in device
	CreateRTV(device);
}
...
{
	/*Present the frame. */
	//CALL Present() on _swapChain passing in 0,0
...
void SwapChain::CreateRTV(ComPtr<ID3D12Device> device)
{
	...
	/* Descriptor(DX12) = View(~DX11)
	 ...
	 //DECLARE a variable called rtvHeapSize of type int32 and ASSIGN it to the return value of device->GetDescriptorHandleIncrementSize() passing in D3D12_DESCRIPTOR_HEAP_TYPE_RTV
